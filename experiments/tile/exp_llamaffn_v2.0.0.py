...
def triton_fused_ffn_loop_split(
    ...
    y1_lhs = tl.sigmoid(y1_lhs) * y1_lhs
    y1_lhs = y1_lhs.to(tl.float16)
    ...
    y1 = y1_lhs * y1_rhs.to(tl.float16)
...
def triton_fused_ffn_atomic(
    ...
    y1 = tl.sigmoid(y1_lhs.to(tl.float32)) * y1_lhs * y1_rhs
    y1 = y1.to(tl.float16)
...
for M in [1, 2, 4, 16]:
    ...
    def benchmark(D, provider):
        ...
        if provider == 'torch_naive':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch_ref(a, w1, w2))
        if provider == 'triton_fused':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_llama_ffn(a, w1, w2))
        if provider == 'triton_fused_loop_split':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_llama_ffn_loop_split(a, w1, w2))
        if provider == 'triton_fused_atomic':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_llama_ffn_atomic(a, w1, w2))
