...
class SoftwarePipelinePass(TileFunctionPass):
    def __init__(self, name=None, num_stages: int = 3):
        super().__init__(name)
    ...
    def process_tile_func(self, func: Function) -> Function:
        ...
        for loop, loads in loop2loads.items():
            ...
            rewriter = SoftwarePipelineRewriter(loop, yield_stmt, loads, dependency_graph, num_stages=self.num_stages)
            func = rewriter(func)
...
def software_pipeline_pass(num_stages=3) -> TileFunctionPass:
    return SoftwarePipelinePass(num_stages=num_stages)
