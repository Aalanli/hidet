        ...
        for BN in [32, 64, 128]:
            for num_warps in [2, 4, 8]:
                for num_stages in [2, 3, 4]:
                    configs.append(Config({'BLOCK_K': BK, 'BLOCK_N': BN, 'BLOCK_O': 64}, num_stages=num_stages, num_warps=num_warps))
    return configs
...
def test():
    ...
    k_cache1 = torch.randn(B, H, LS, DH, dtype=torch.float16, device='cuda')
    v_cache1 = torch.randn(B, H, LS, DH, dtype=torch.float16, device='cuda')

    k_cache2 = k_cache1.clone()
    v_cache2 = v_cache1.clone()
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        plot_name=f"attn_performance-H={H}-D={D}-DH={DH}-S={S}",  # Name for the plot, used also as a file name for saving the plot.
        args={},
    )
)
def benchmark(L, provider):
    ...
    if provider == 'torch':
        if triton.__version__.split('.')[0] == '1':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: ref_decoding_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH))
        elif triton.__version__.split('.')[0] == '2':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: ref_decoding_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH), quantiles=quantiles)
    if provider == 'triton':
        if triton.__version__.split('.')[0] == '1':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_decoding_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH))
        elif triton.__version__.split('.')[0] == '2':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_decoding_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH), quantiles=quantiles)
    if provider == 'triton-flash':
        if triton.__version__.split('.')[0] == '1':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_flash_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH))
        elif triton.__version__.split('.')[0] == '2':
            ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_flash_attn(x, wq, wk, wv, wo, k_cache, v_cache, L, H, DH), quantiles=quantiles)
