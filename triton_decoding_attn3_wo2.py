    ...
    def benchmark(L, provider):
        ...
        if provider == 'torch':
            ...
            if triton.__version__.split('.')[0] == '1':
                ms, min_ms, max_ms = triton.testing.do_bench(lambda: ref_decoding_attn2(x, wqkv, wo, k_cache, v_cache, L, H, DH))
            elif triton.__version__.split('.')[0] == '2':
                ms, min_ms, max_ms = triton.testing.do_bench(lambda: ref_decoding_attn2(x, wqkv, wo, k_cache, v_cache, L, H, DH), quantiles=quantiles)
        if provider == 'triton':
            if triton.__version__.split('.')[0] == '1':
